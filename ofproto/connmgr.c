...
struct whitelist_arr
{
    struct real_client arr[MAX_SIZE_wHITE_LIST];
    int idx;
};

static struct whitelist_arr wlist = {.idx = 0};

// end test global value

// test static funtion declare
static uint32_t cookie_hash(uint32_t saddr, uint32_t daddr, uint16_t sport, uint16_t dport,
		       uint32_t count, uint32_t c);

static uint32_t secure_tcp_syn_cookie(uint32_t saddr, uint32_t daddr, uint16_t sport,
				   uint16_t dport, uint32_t sseq, uint32_t data);

static uint32_t check_tcp_syn_cookie(uint32_t cookie, uint32_t saddr, uint32_t daddr,
				  uint16_t sport, uint16_t dport, uint32_t sseq);
 // end test function declare
...
/**
 * Test topology 3 host, 1 switch:
 *   - Normal client h1: s1-eth1
 *   - Attacker client h3: s1-eth3
 *   - Server h2: s1-eth2
*/
...
void send_syn_ack_packet(struct dp_packet packet_in)
{
    ...
    uint32_t seq = ntohl(get_16aligned_be32(&tcp->tcp_seq));
    uint32_t syncookie = secure_tcp_syn_cookie(ntohl(flow_in.nw_src), ntohl(flow_in.nw_dst)
                                                    ,ntohs(flow_in.tp_src),ntohs(flow_in.tp_dst),seq,syncookie_secret[1]);
    ...
    if (strcmp(inet_ntoa(sip), "10.0.0.1") == 0)
        strncpy(bufIfreq.ifr_name, "s1-eth1", 16 - 1); // normal client
    else 
        strncpy(bufIfreq.ifr_name, "s1-eth3", 16 - 1); // attacker client
    ...
    if (strcmp(inet_ntoa(sip), "10.0.0.1") == 0)
        ...
        strncpy(bufIfreq.ifr_name, "s1-eth3", 16 - 1);
    ...
    else
    {
        ...
        tcph->seq = htonl(syncookie);
        tcph->ack_seq = htonl(ntohl(get_16aligned_be32(&tcp->tcp_seq)) + 1);
        // fprintf(log, "Seq number of syn/ack packet  host: %u, net: %u\n",syncookie, tcph->seq );
...
// add flow khi nhan duoc goi ACK tu server h2
void add_myflow_h1_to_h2(struct connmgr *mgr, struct flow tflow)
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    tmatch_10.wildcards = htonl(OFPFW10_DL_VLAN) | htonl(OFPFW10_DL_VLAN_PCP) | htonl(OFPFW10_NW_TOS) | htonl(OFPFW10_TP_SRC);

    tmatch_10.dl_vlan = htons(0);
    tmatch_10.dl_vlan_pcp = 0;
    tmatch_10.dl_src = tflow.dl_dst;
    tmatch_10.dl_dst = tflow.dl_src;
    tmatch_10.dl_type = ofputil_dl_type_to_openflow(tflow.dl_type);
    tmatch_10.nw_src = tflow.nw_dst;
    tmatch_10.nw_dst = tflow.nw_src;
    tmatch_10.nw_tos = tflow.nw_tos & IP_DSCP_MASK;
    tmatch_10.nw_proto = tflow.nw_proto;
    tmatch_10.tp_src = tflow.tp_dst;
    tmatch_10.tp_dst = tflow.tp_src;
    memset(tmatch_10.pad1, '\0', sizeof tmatch_10.pad1);
    memset(tmatch_10.pad2, '\0', sizeof tmatch_10.pad2);

    if (ntohl(tflow.nw_dst) == 167772161)
    {
        tmatch_10.in_port = htons(1);
    }
    else 
    {
        tmatch_10.in_port = htons(3);
    }
    ...
    // {
    //     ofpact_put_OUTPUT(&ofpacts)->port = 1; // chuyen goi ra port ung voi L2/L3
    // }
    ...
    ofpact_put_OUTPUT(&ofpacts)->port = 2; // chuyen goi ra port 2 ung voi server
...
void add_myflow_h2_to_h1(struct connmgr *mgr, struct flow tflow)
    OVS_EXCLUDED(ofproto_mutex)
{
    // tao 1 empty flow cho cho openflow 10
    struct ofp10_match tmatch_10;
    ...
    // setup cac truong can thiet cho openflow 10
    //  tmatch_10.wildcards = OFPFW10_NW_PROTO | OFPFW10_DL_SRC | OFPFW10_DL_DST | OFPFW10_NW_SRC_ALL | OFPFW10_NW_DST_ALL | OFPFW10_TP_SRC | OFPFW10_TP_DST;
    tmatch_10.wildcards = htonl(OFPFW10_DL_VLAN) | htonl(OFPFW10_DL_VLAN_PCP) | htonl(OFPFW10_NW_TOS) | htonl(OFPFW10_TP_DST);
    ...
    {
        ofpact_put_OUTPUT(&ofpacts)->port = 3;
    }
...
static uint32_t cookie_hash(uint32_t saddr, uint32_t daddr, uint16_t sport, uint16_t dport,
		       uint32_t count, uint32_t c)
{
    // lay gia tri random cho khoa bi mat 
    // syncookie_secret[0] = random_uint32(); 
    // syncookie_secret[1] = random_uint32(); 
				   ...
				   uint16_t dport, uint32_t sseq, uint32_t data)
{
    ...
    // FILE *log = fopen("/home/aothatday1/Desktop/log_file.txt", "a");
    // fprintf(log, "count when send syn/ack = %u\n",count);
    // fclose(log);
	return (cookie_hash(saddr, daddr, sport, dport, 0, 0) + sseq + 
                ((uint32_t)(count << COOKIEBITS) + (uint32_t)((cookie_hash(saddr, daddr, sport, dport, count, 1) + data) & COOKIEMASK)));
}
				  ...
				  uint16_t sport, uint16_t dport, uint32_t sseq)
{
    ...
    // FILE *log = fopen("/home/aothatday1/Desktop/log_file.txt", "a");
    // fprintf(log, "diff = %u,count when receive ack = %u,count when send syn/ack: %u\n",diff,count,(uint32_t)(cookie >> COOKIEBITS));
    // fprintf(log, "Check result = %u\n",(cookie - cookie_hash(saddr, daddr, sport, dport, count - diff, 1))& COOKIEMASK);
    // fclose(log);
...
// return 0 neu tim thay, nguoc lai tra ve -1
int check_white_list(ovs_be16 tcp_src, uint32_t ip_src)
{
    for(int i=0; i <=  wlist.idx; i++)
        {
            if( (tcp_src == wlist.arr[i].tcp_src) && (ip_src == wlist.arr[i].ip_src ))
                return 0;
        }
    
    return -1; // khong tim thay trong white list
};
                            ...
                            const struct ofproto_async_msg *am)
{
    ...
    // static struct dp_packet *uncheck_syn_packet;
    // static struct flow uncheck_syn_flow;

    // static bool check_conn = false; // bien nay xac dinh 1 ket noi TCP thanh cong
    ...
    // xu ly goi ACK
    else if (htons(tflow.tcp_flags) == 16 )
    {
        struct tcp_header *tcp = dp_packet_l4(tpacket);
        // struct ip_header *ip = dp_packet_l3(tpacket);
        ...
        // FILE *log = fopen("/home/aothatday1/Desktop/log_file.txt", "a");
        // fprintf(log, "This is ACK packet with syn/ack, host:%u, net:%u \n.......................................................\n",  ntohl(get_16aligned_be32(&tcp->tcp_ack)), get_16aligned_be32(&tcp->tcp_ack));
        // fclose(log);
                                        ...
                                        ,ntohs(tflow.tp_src), ntohs(tflow.tp_dst),sseq) == syncookie_secret[1])
        {
            ...
            wlist.arr[wlist.idx].tcp_src = tflow.tp_src;
            wlist.arr[wlist.idx].ip_src = tflow.nw_src;
            wlist.idx +=1;
            // log = fopen("/home/aothatday1/Desktop/log_file.txt", "a");
            // fprintf(log, "Luu vao whitelist voi idx:%u, port:%u, ip:%u  \n\n",wlist.idx,wlist.arr[wlist.idx-1].tcp_src,wlist.arr[wlist.idx-1].ip_src);
            // fclose(log);
    ...
    // xu ly goi SYN/ACK
    else if (htons(tflow.tcp_flags) == 18 )
    {
        // FILE *log = fopen("/home/aothatday1/Desktop/log_file.txt", "a");
        ...
        // SYN/ACK packet gui den 1 dia chi trong white list thi moi cho qua
        if(check_white_list(tflow.tp_dst,tflow.nw_dst) == 0)
        {
             send_ack_packet(*tpacket);

            // add flow thu cong

            add_myflow_h1_to_h2(mgr, tflow);
            add_myflow_h2_to_h1(mgr, tflow);
    ...
    else if (htons(tflow.tcp_flags) == 24)
    {
        // FILE *log = fopen("/home/aothatday1/Desktop/log_file.txt", "a");
        ...
        // drop PSH/Ack packet which not come in from whitelist
        if(check_white_list(tflow.tp_src,tflow.nw_src) == 0)
            check = true;
        else check = false;
    }
